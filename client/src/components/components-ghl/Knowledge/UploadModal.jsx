...
const UploadModal = ({ isOpen, onClose }) => {
  ...
  const handleFinalUpload = (name) => {
    console.log("Finalizing upload with name:", name);
    handleClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      <div className={`bg-white rounded-xl shadow-xl w-full transition-all duration-300 overflow-hidden ${
        currentStep === 'selection' ? 'max-w-lg' : 
          ...
          <>
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div className="flex items-center gap-2">
                ...
                <div className="w-5 h-5 border border-gray-300 rounded-full flex items-center justify-center text-[10px] text-gray-500 font-bold cursor-help">i</div>
              </div>
            ...
            <div className="p-6 space-y-3">
              {[
                { id: 'text', title: 'text', desc: 'Input plain text to be stored.', icon: <Type /> },
                { id: 'faq', title: 'faq', desc: 'Input FAQs to be stored.', icon: <ListOrdered /> },
                { id: 'file', title: 'file', desc: 'Upload a file to be stored.', icon: <FolderUp /> },
                { id: 'url', title: 'url', desc: 'Scrape a URL to store data.', icon: <Globe /> },
              ].map((opt) => (
                <div
                  key={opt.id}
                ...
                >
                  ...
                  <div className="flex-grow">
                    <h3 className="font-bold text-gray-900 capitalize">{opt.title}</h3>
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t bg-gray-50/50">
              <button onClick={handleClose} className="px-5 py-2 text-gray-600 font-medium hover:text-gray-800">
                Close
              </button>
            </div>
          </>
