...
import { Search, ChevronLeft, ChevronRight, Ban, Home, Upload, ShoppingCart, Loader2, Phone } from 'lucide-react';
...
const Numbers = () => {
      ...
      if (subaccountId && assistants.length > 0) {
        ...
        try {
          const promises = assistants.map(assistant => 
            dispatch(fetchPurchasedNumbers({ 
              subaccountId, 
  ...
  const getFilteredNumbers = () => {
    switch (activeTab) {
      case 'bought':
        return allPurchasedNumbers.filter(num => 
          num.phoneNumberDetails?.origin === 'twilio'
        );
      case 'imported':
        return allPurchasedNumbers.filter(num => 
          num.phoneNumberDetails?.origin !== 'twilio'
  ...
  const headers = ['NAME', 'NUMBER', 'UPDATED', 'CAPABILITIES', 'STATUS', 'LINKED ASSISTANT'];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex border-b border-gray-200 mb-6 bg-white rounded-t-lg shadow-sm">
          ...
          <TabButton isActive={activeTab === 'bought'} onClick={() => setActiveTab('bought')}>
            Bought {allPurchasedNumbers.filter(n => n.phoneNumberDetails?.origin === 'twilio').length}
          </TabButton>
          <TabButton isActive={activeTab === 'imported'} onClick={() => setActiveTab('imported')}>
            Imported {allPurchasedNumbers.filter(n => n.phoneNumberDetails?.origin !== 'twilio').length}
          </TabButton>
        </div>
                ...
                <tr>
                  <td colSpan={headers.length} className="px-6 py-12 text-center">
                    <div className="flex items-center justify-center">
                      ...
                      <span className="text-gray-600">Loading purchased numbers...</span>
                    </div>
                  </td>
                </tr>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200">
          <div className="flex items-center space-x-2 text-sm text-gray-700">
            ...
            <span className="font-medium text-gray-500">{filteredNumbers.length} Results</span>
          </div>
